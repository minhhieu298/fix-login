...
import { getSwitchTabLogin } from "./constants";
import { ISwitchTab, TabPanelProps } from "./types";

const CustomSwitchTab = styled(Tabs)(({ theme }) => ({
  minHeight: 36,
  height: 36,
  ...
  ".MuiButtonBase-root": {
    ...
    path: {
      fill: theme.palette.customBase?.base20,
      ...theme.applyStyles("dark", {
        fill: theme.palette.customBase?.base60,
      }),
    },
    "&.Mui-selected": {
      path: {
        fill: theme.palette.customSupport?.greendefault03,
      },
      ".custom-text": {
...
// custom nút switch tab giữa login và QR code
export const SwitchTypeLogin = ({ value, onChange }: ISwitchTab) => {
  ...
  return (
    ...
    >
      {getSwitchTabLogin(t).map((item, ind) => (
        <Tab
          disableTouchRipple
          disableFocusRipple
          disableRipple
...
export const CustomBoxFooter = styled(Stack)(() => ({
  alignItems: "center",
  gap: 8,
  cursor: "pointer",
  flexDirection: "row",
...
export const EyeVisibleOff: FC<IComponentProps<HTMLOrSVGElement>> = (props) => {
  return (
    <svg
      width={props.width || "24"}
      height={props.height || "24"}
      viewBox="0 0 24 24"
      fill={props.color || "#858B93"}
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        fillRule="evenodd"
...
export const EyeVisible: FC<IComponentProps<HTMLOrSVGElement>> = (props) => {
  return (
    <svg
      width={props.width || "24"}
      height={props.height || "24"}
