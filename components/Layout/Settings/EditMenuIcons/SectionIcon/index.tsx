...
import { allIcon, renderNameTitle } from "../constant";
import styles from "../editMenuIcons.module.css";
import { IMenuBarIcon } from "../interface";

const SectionIconComponent: React.FC<{
  ...
  const sectionIcon =
    allIcon[name as keyof typeof allIcon].filter(
      (icon: IMenuBarIcon) => !arrayIcon.includes(icon.id)
    ) || [];
  ...
  return (
    ...
    >
      <Typography variant="body14-B21">{t(renderNameTitle(name))}</Typography>
      <Box
        sx={{
          display: "flex",
          flexWrap: "wrap",
...
export const SectionIcon = memo(SectionIconComponent);
